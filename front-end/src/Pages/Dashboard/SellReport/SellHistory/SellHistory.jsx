...
const SellHistory = () => {
  ...
  const totalDiscount =
    filteredData?.length > 0
      ? filteredData.reduce(
          (sum, item) => item?.invoice[0].totalDiscount + sum,
          0
        )
      : 0;
        ...
        {filteredData?.length > 0 ? (
          <table className="overflow-x-scroll mx-auto sm:max-w-full md:max-w-full border-collapse w-full">
            <tr className="bg-gray-300 w-full">
              <th className="hidden md:block text-center p-2 border border-white">
                Invoice Id
              </th>
              <th className="text-center p-2 border border-white w-[40%]">
                Date
              </th>

              <th className="text-center p-2 border border-white w-[10%]">
                Price
              </th>
              <th className="text-center p-2 border border-white w-[10%]">
                Discount
            ...
            {filteredData?.map((data, index) => (
              <tr key={index} className="bg-blue-50 w-full">
                <td className="hidden md:block text-center p-2 border border-white">
                  {data?._id}
                </td>
        ...
        <div className="max-w-[25rem] ml-auto pr-8 space-y-2 py-4">
          ...
          <div className="border-b border-gray flex items-center justify-between">
            ...
            <p className="font-bold">
              {(beforeDiscountPrice - totalDiscount).toFixed(2)}
            </p>
          </div>
        </div>
      ) : null}
